...
Covers all patterns from VIDESKTOP_SCRAPER_GUIDE.md:
  - §3  Navigation: organic discovery, entry-URL + link text, direct portal
  - §4  Grid structure, row href capture (3-strategy fallback)
  - §5  Keyword search + silent networkidle
  - §6  Pagination (Next button detection)
  - §7  Row clicking, new-tab handling, RecJobView confirmation
  - §8  RecApplicantEmail.aspx?Tag=UUID is unique per job (acceptable job URL)
  - §9  Timeout standards
  - §10 Popup / cookie-banner dismissal (11 selectors)
  - §11 Location validation rules
  - §12 Salary extraction (6 patterns, K-suffix, hourly conversion)
  - §13 Title blocklist filtering
  - §14 Deduplication key
  - §18 Edge cases (expired Tag URL, empty grid, postback failures)
...
Links you must NEVER click during navigation:
  ✗ "Attorneys", "Lawyers", "Associates", "Partners", "Of Counsel", "Legal Careers"
  ✗ "Alumni", "Diversity", "Pro Bono", "Newsroom", "Press", "Events", "About", "People"
  ✗ Any link to a PDF, news article, or press release
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ CRITICAL — VIDESKTOP POSTBACK BEHAVIOUR (read before doing anything):
...
For EACH candidate in your list:
  ...
  CASE B — pre_click_job_url is None (Option 4 — no ID found before click):
    ...
    SUB-STEP B2 — Extract the real job URL from the post-click page HTML:
      ...
      Search for these href patterns IN THIS ORDER:
        ...
        PATTERN 1 (best — always present on detail view):
          ...
          ✓ This is unique per job — use this as the job URL.
        ...
        PATTERN 2 (fallback — only if Pattern 1 not found):
          ...
          ⚠️ RecJobView may 404 on some portals — prefer RecApplicantEmail when both exist.
    ...
    ⚠️ If NEITHER pattern is found in the page HTML after the postback:
  ...
  VALIDATION — before proceeding to extraction:
    ✓ VALID job_url: contains "RecJobView" or "RecApplicantEmail"
    ✗ INVALID job_url: contains "RecDefault" → you have the listing URL, NOT a job URL.
...
SALARY EXTRACTION — apply these patterns in order (stop at first match):
  ...
  salary_raw: ⚠️ the EXACT raw salary sentence as written on the page (plain text string only).
  ...
  is_hourly:  true  — if and only if the page explicitly states the salary as an hourly rate
              ...
              ⚠️ Set true ONLY when the page uses "per hour", "/hr", "/hour", or "hourly" wording.
...
JOB URL
  job_url: The role-specific URL for this job — extracted from row HTML or detail page HTML.
           ⚠️ viDesktop uses ASP.NET postbacks: page.url NEVER changes after clicking a row.
              page.url stays at RecDefault.aspx — NEVER use page.url as job_url.
           ✓ VALID: contains "RecJobView.aspx"        — unique job detail page
           ✓ VALID: contains "RecApplicantEmail.aspx" — unique apply page with job description
           ❌ INVALID: contains "RecDefault.aspx"     — listing page, NOT a job URL
           ❌ INVALID: is the starting careers page URL ({url})
